...
train_member = pd.read_csv("../data/processed_ml-1m/shadow_member.csv")
member_interacted_dict = train_member.groupby('userId')['itemId'].apply(list).to_dict()
#print(member_interacted_dict[1])
train_member_recommendation = pd.read_csv("../data/processed_ml-1m/processed_Lml1m/shadow_recommendations.csv")
train_member_recommendation_dict = train_member_recommendation.set_index('raw_uid').apply(list, axis=1).to_dict()
#print(train_member_recommendation_dict[1])

item_matrix = pd.read_csv("../data/processed_ml-1m/mf_item_matrix.csv")
item_embedding_dict = item_matrix.set_index('raw_iid').iloc[:, 1:].apply(list, axis=1).to_dict()
#print(item_embedding_dict[3421])
...
for user_id, item_list in member_interacted_dict.items():
    for item in item_list:
        ...
        for rec_item in train_member_recommendation_dict[user_id]:
            ...
            rank += 1
#print(train_data_member[(1,1193)])
#print(len(train_data_member[(1,1193)]))

print('---------------------non-member------------------------')

train_nonmember = pd.read_csv("../data/processed_ml-1m/shadow_nonmember")
nonmember_interacted_dict = train_nonmember.groupby('userId')['itemId'].apply(list).to_dict()
#print(nonmember_interacted_dict[3230])

train_nonmember_recommendation = pd.read_csv("../data/processed_ml-1m/processed_Lml1m/shadow_nonmember_recommendation.csv")
train_nonmember_recommendation_dict = train_nonmember_recommendation.groupby('userId')['itemId'].apply(list).to_dict()
#print(train_nonmember_recommendation_dict[3230])
...
for user_id, item_list in nonmember_interacted_dict.items():
    for item in item_list:
        ...
        for rec_item in train_nonmember_recommendation_dict[user_id]:
            ...
            rank += 1
#print(train_data_nonmember[(3230,1729)])
#print(len(train_data_nonmember[(3230,1729)]))


#create attack file
dir_path = '../CIKM_Attack'
with open(dir_path + "/train_ML.txt", 'w') as f:
