...
def train_model(train_file, test_file, target_fpr):
    ...
    with torch.no_grad():
        best_threshold, best_tpr, best_fpr, global_auc = evaluate_model_with_roc_curve(model, test_data, test_labels, target_fpr)
        return best_threshold, best_tpr, best_fpr, global_auc
...
# 数据路径和文件
for char in ['BL','DL']:
    train_file = path + "/trainForClassifier_" + char + ".txt"
    test_file = path + "/testForClassifier_" + char + ".txt"
