...
train_member = pd.read_csv("../data/processed_ml-1m/target_member.csv")
member_interacted_dict = train_member.groupby('userId')['itemId'].apply(list).to_dict()
print(member_interacted_dict[2])
train_member_recommendation = pd.read_csv("../data/processed_ml-1m/processed_Nml1m/target_recommendations.csv")
train_member_recommendation_dict = train_member_recommendation.set_index('raw_uid').apply(list, axis=1).to_dict()
print(train_member_recommendation_dict[2])
...
for user_id, item_list in member_interacted_dict.items():
    for item in item_list:
        ...
        for rec_item in train_member_recommendation_dict[user_id]:
            ...
            rank += 1
print(train_data_member[(2,1357)])
print(train_data_member[(2,1357)])

print('---------------------non-member------------------------')

train_nonmember = pd.read_csv("../data/processed_ml-1m/target_nonmember")
nonmember_interacted_dict = train_nonmember.groupby('userId')['itemId'].apply(list).to_dict()
print(nonmember_interacted_dict[2927])

train_nonmember_recommendation = pd.read_csv("../data/processed_ml-1m/processed_Nml1m/target_nonmember_recommendation.csv")
train_nonmember_recommendation_dict = train_nonmember_recommendation.groupby('userId')['itemId'].apply(list).to_dict()
print(train_nonmember_recommendation_dict[2927])
...
for user_id, item_list in nonmember_interacted_dict.items():
    for item in item_list:
        ...
        for rec_item in train_nonmember_recommendation_dict[user_id]:
            ...
            rank += 1
print(train_data_nonmember[(2927,1376)])
print(train_data_nonmember[(2927,1376)])


#create attack file
dir_path = '../CIKM_Attack'
with open(dir_path + "/test_MN.txt", 'w') as f:
